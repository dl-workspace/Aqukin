    ...
    switch (args.getSubcommand()) {
      case INFO_OPTIONS.user: {
        ...
        if (user) {
          ...
          interaction.followUp({
            embeds: [
              await createUserInfoEmbed(interaction.user, interaction.member),
            ],
          });
        }
      ...
      case INFO_OPTIONS.server: {
        interaction.followUp({
          embeds: [await createGuildinfoEmbed(interaction.guild)],
        });
        break;
      }
    }
...
async function handleBotInfo({ client, interaction, args }: ExecuteOptions) {
  const embed = baseEmbed()
    ...
    .addFields(
      ...
      {
        name: "Description",
        value: `Your (un)reliable Super Idol Gamer Maid♥, a bot that was created based on a Virtual Youtuber known as **Minato Aqua**`,
      },
      {
        name: "Version",
  ...
  const actionRow =
    new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents([
      ...
      new ButtonBuilder()
        ...
        .setURL(
          "https://www.youtube.com/channel/UC1opHUrw8rvnsadT-iGp7Cg?view_as=subscriber?sub_confirmation=1"
        )
        .setStyle(ButtonStyle.Link),
      new ButtonBuilder()
        .setLabel("@minatoaqua")
...
async function createGuildinfoEmbed(guild: Guild) {
  return baseEmbed()
    ...
    .addFields(
      ...
      {
        name: "Boosting Level",
        value: guild.premiumTier.toLocaleString(),
        inline: true,
      },
      {
        name: "Boosting Count",
      ...
      },
      {
        ...
        value: guild.verificationLevel.toLocaleString(),
        inline: true,
      },
      {
        name: "NSFW Level",
        value: guild.nsfwLevel.toLocaleString(),
        inline: true,
      },
      {
        name: "Primary Lanague",
        value: guild.preferredLocale.toLocaleString(),
        inline: true,
      },
      {
        name: "Joined At",
        value: guild.joinedAt.toLocaleDateString(),
        inline: true,
      },
      {
        name: "Create At",
      ...
      {
        name: "Owner",
        value: `${(await guild.fetchOwner()).user.username}-sama`,
        inline: true,
      }
