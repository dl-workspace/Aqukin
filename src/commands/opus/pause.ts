...
export default new Command({
  ...
  execute: async ({ client, interaction, args, mPlayer }) => {
    if (mPlayer.subscription.player.state.status === AudioPlayerStatus.Paused) {
      ...
      mPlayer.subscription.player.state.status === AudioPlayerStatus.Playing
    ) {
      mPlayer.subscription.player.pause();
      interaction.followUp({
        content: client.replyMsgAuthor(
          interaction.member,
          `${client.user.username} has paused audio streaming`
        ),
      }); // at \`${formatDuration(mPlayer.queue[0].remainingTime())}\`
