            ...
            case LOOP_OPTIONS.track:
                if(mPlayer.isLoopingQueue()){
                    ...
                    const actionRow = new ActionRowBuilder<MessageActionRowComponentBuilder>()
                        .addComponents([
                            new ButtonBuilder()
                                .setCustomId(generateInteractionComponentId(interaction.user.id, LOOP_OPTIONS.disableLoopQueue_yes, times))
                                .setLabel('Yes')
                                .setStyle(ButtonStyle.Success),
                            new ButtonBuilder()
                                .setCustomId(generateInteractionComponentId(interaction.user.id, LOOP_OPTIONS.loop_no))
                                .setLabel('No')
            ...
            case LOOP_OPTIONS.queue:
                if(mPlayer.isLoopingTrack()){
                    ...
                    const actionRow = new ActionRowBuilder<MessageActionRowComponentBuilder>()
                        .addComponents([
                            new ButtonBuilder()
                                .setCustomId(generateInteractionComponentId(interaction.user.id, LOOP_OPTIONS.disableLoopTrack_yes, times))
                                .setLabel('Yes')
                                .setStyle(ButtonStyle.Success),
                            new ButtonBuilder()
                                .setCustomId(generateInteractionComponentId(interaction.user.id, LOOP_OPTIONS.loop_no))
...
export async function loopTrack(client: ExtendedClient, mPlayer: OpusPlayer, interaction: ButtonInteraction | ExtendedInteraction, times: number){
    mPlayer.enableTrackRepeat(times);
    mPlayer.updatePlayingStatusMsg();
    await interaction.editReply({ content: client.replyMsgAuthor((interaction.member as GuildMember), `${client.user.username} will now \`loop\` the current \`track\` ${ times == -1 ? '\`indefinitely\`' : `\`${times} time(s)\`` }`), embeds: [], components: [] });
}
...
export async function loopQueue(client: ExtendedClient, mPlayer: OpusPlayer, interaction: ButtonInteraction | ExtendedInteraction, times: number){
    mPlayer.enableQueueRepeat(times);
    mPlayer.updatePlayingStatusMsg();
    await interaction.editReply({ content: client.replyMsgAuthor((interaction.member as GuildMember), `${client.user.username} will now \`loop\` the current \`queue\` ${ times == -1 ? '\`indefinitely\`' : `\`${times} time(s)\`` }`), embeds: [], components: [] });
}